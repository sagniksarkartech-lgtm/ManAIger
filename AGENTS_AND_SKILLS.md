...
*   `name`: A unique identifier (e.g., "EmailAgent").
...
The `RouterAgent` acts as the central dispatch hub for all unstructured data entering the system. 
...
Specialized agents handle the heavy lifting of interacting with the LLM and processing specific data domains.
...
Instead of an agent calling an external API to send an email or authorize a payment, it must invoke a custom Python skill: `queue_for_human_approval`.

### How the Intercept Skill Works:
1.  **Halt Execution:** The specialized agent drafts the proposed action (e.g., the JSON payload for a drafted email response).
...
This architectural hard-stop guarantees that ManAIger remains an *autonomous-drafting* system rather than an *autonomous-executing* system, ensuring zero risk of unapproved enterprise actions.